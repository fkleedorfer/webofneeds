...
public enum UnboundHandlingMode {
    ...
    ALL_BOUND,
    /**
     * bl:unbound is always an option but may not be in a solution if there is
     * another solution in which it's bound
     */
    ...
    /**
     * Informs the client whether or not bl:unbound should be considered an option,
     * <code>optionsArePresent</code> being what it is.
     * 
    ...
    public boolean isUnboundAnOption(boolean optionsArePresent) {
        switch (this) {
            ...
            case ALLOW_UNBOUND:
                return true;
            case UNBOUND_ALLOWED_IF_NO_OTHER_BINDING:
                return true;
            case UNBOUND_ALLOWED_IF_NO_OTHER_OPTION:
                return !optionsArePresent;
        }
