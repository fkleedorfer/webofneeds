...
public class BindingSetExtractingShaclPaths {
    ...
    /**
     * Copy a SHACL path from one graph to another. Return the path head to be
     * connected to the rest of the graph. This function does not preserve blank
     * nodes in the SHACL path.
     */
    public static Node copyPath(Graph srcGraph, Graph dstGraph, Node start) {
        Path resultPath = BindingSetExtractingShaclPaths.parsePath(srcGraph, start);
        Node pn = BindingSetExtractingShaclPaths.pathToRDF(resultPath, dstGraph);
        return pn;
    }
    ...
    /*
     * P_Link P_Inverse P_Alt P_Seq P_OneOrMore1, P_OneOrMoreN P_ZeroOrMore1,
     * P_ZeroOrMoreN P_ZeroOrOne
     */
    /*
     * SPARQL Property path: ex:parent SHACL Property path: ex:parent SPARQL
     * Property path: ^ex:parent SHACL Property path: [ sh:inversePath ex:parent ]
     * SPARQL Property path: ex:parent/ex:firstName SHACL Property path: ( ex:parent
     * ex:firstName ) SPARQL Property path: rdf:type/rdfs:subClassOf* SHACL Property
     * path: ( rdf:type [ sh:zeroOrMorePath rdfs:subClassOf ] ) SPARQL Property
     * path: ex:father|ex:mother SHACL Property path: [ sh:alternativePath (
     * ex:father ex:mother ) ]
     */
    ...
    static class PathToRDF implements PathVisitor {
        ...
        public void visit(P_Shortest pathShortest) {
            throw new ShaclParseException("Not part of SHACL: " + pathShortest);
        }

        @Override
        public void visit(P_Multi pathMulti) {
            throw new ShaclParseException("Not part of SHACL: " + pathMulti);
        }

        @Override
        public void visit(P_Distinct pathDistinct) {
            throw new ShaclParseException("Not part of SHACL: " + pathDistinct);
        }

        @Override
        public void visit(P_FixedLength pFixedLength) {
            throw new ShaclParseException("Not part of SHACL: " + pFixedLength);
        }

        @Override
        public void visit(P_Mod pathMod) {
            throw new ShaclParseException("Not part of SHACL: " + pathMod);
        }

        @Override
        public void visit(P_NegPropSet pathNotOneOf) {
            throw new ShaclParseException("Not part of SHACL: " + pathNotOneOf);
        }
    }
}
